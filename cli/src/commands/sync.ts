        ...
        .action(async (localDir: string, options: any) => {
            ...
            try {
                ...
                if (options.folder) {
                    ...
                    const existingFolder = folders.find((f: any) => f.name.toLowerCase() === folderName.toLowerCase());
                ...
                const newestRemoteFilesMap = new Map<string, any>();
                const allRemoteFilesGrouped = new Map<string, any[]>();
                ...
                let autoEnv = "local";
                if (process.env.GITHUB_ACTIONS) autoEnv = "github-ci";
                else if (process.env.CI) autoEnv = "ci";
                else if (process.env.NODE_ENV) autoEnv = process.env.NODE_ENV;
                ...
                const artifactMetadata = {
                    ...
                    tags: (options.tags || process.env.FYLO_TAGS || "").split(",").map((t: string) => t.trim()).filter(Boolean),
                ...
                const uploadsQueue: { filePath: string; relativePath: string; reason: string; size: number; oldFileIds: string[] }[] = [];
                ...
                for (const filePath of localFilePaths) {
                    ...
                    if (!remoteFile) {
                        ...
                        if (remoteFile) remoteNameMatch = relativePath + ".gz";
                    }
                    if (!remoteFile) {
                        remoteFile = newestRemoteFilesMap.get(relativePath + ".zip");
                        if (remoteFile) remoteNameMatch = relativePath + ".zip";
                    }
                    ...
                    if (!remoteFile) {
                        ...
                        if (!sizeMatches || !checksumMatches) {
                            ...
                            const oldFileIds = oldFileRecords.map((r: any) => r.id);
                ...
                const deleteQueue: { fileId: string; remoteName: string }[] = [];
                ...
                const result = {
                    ...
                    skipped: skipCount,
                    uploaded: [] as { file: string; size: number; id?: string; reason: string }[],
                    deleted: [] as { file: string; id: string }[],
                };
