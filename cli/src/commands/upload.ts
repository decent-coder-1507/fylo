...
export function getPackageMetadata(filePath: string): { name?: string; version?: string } {
    try {
...
export async function runUploadForFile(
    filePath: string,
    folderId: string | undefined,
    artifactMetadata: any,
    options: { session?: string | boolean; customFileName?: string } = {}
): Promise<any> {
    const stats = fs.statSync(filePath);
