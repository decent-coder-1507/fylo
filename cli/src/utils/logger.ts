...
class Logger {
     ...
     */
    public output<T>(result: T, prettyPrinter: (data: T) => void) {
        if (this.isJsonMode) {
     ...
     */
    public fatal(msg: string, err?: any, exitCode = 1): never {
        this.error(msg, err);
